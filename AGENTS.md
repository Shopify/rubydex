...
Rubydex collects diagnostics during its core analysis (built-in rules) and also allows consumers to define their own
custom rules.

- Built-in rules are auto-generated by running `bundle exec rake generate_rules` and live in `lib/rubydex/rules`. Their
  original definition lives on the Rust side in `rust/rubydex/src/diagnostic.rs`
- Custom rules (including the ones we bundle into rubydex) are manually defined in `lib/rubydex_linter/rules`
