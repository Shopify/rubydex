...
impl Graph {
    ...
    fn invalidate(&mut self, old_document: Option<&Document>, new_local_graph: Option<&LocalGraph>) {
        ...
        let mut pending_detachments: IdentityHashMap<DeclarationId, Vec<DefinitionId>> =
    ...
    ///
    /// Detaches any pending definitions first, then decides the mode:
    /// - Remove: no definitions remain or owner was already removed (orphaned)
    /// - Ancestor-stale: declaration survives but its ancestor chain needs refreshing
            ...
            for dep in deps {
                match dep {
                    NameDependent::ChildName(id) | NameDependent::NestedName(id) => {
                        queue.push(InvalidationItem::UnresolveName(*id));
                    }
            ...
            for dep in deps {
                match dep {
                    ...
                    NameDependent::NestedName(id) => {
                        queue.push(InvalidationItem::UnresolveReferences(*id));
                    }
                    NameDependent::Reference(_) | NameDependent::Definition(_) => {}
                }
            }
...
mod tests {
    ...
    use crate::{
        ...
        assert_declaration_references_count_eq, assert_dependents, assert_descendants, assert_members_eq,
    ...
    // NOTE: deleting_sole_definition_file_cascades_removal_through_nested_declarations
    // requires incremental resolution (Resolver::resolve) to pass. Deferred to the
    // incremental resolver PR.

    // ==========================================
    // Incremental invalidation tests
    //
    // These assert graph state AFTER invalidation but BEFORE re-resolve,
    // verifying that the invalidation engine correctly cascades changes.
    // ==========================================
