...
impl Graph {
    ...
    fn unresolve_reference(&mut self, reference_id: ConstantReferenceId) -> Option<DeclarationId> {
        ...
        if let Some(old_decl_id) = self.unresolve_name(name_id) {
            match self.declarations.get_mut(&old_decl_id) {
                Some(Declaration::Namespace(it)) => {
                    it.remove_reference(&reference_id);
                }
                Some(Declaration::Constant(it)) => {
                    it.remove_reference(&reference_id);
                }
                Some(Declaration::ConstantAlias(it)) => {
                    it.remove_reference(&reference_id);
                }
                Some(
                    ...
                    unreachable!("Cannot unresolve reference for something that isn't a constant")
                }
                None => {
                    panic!("Tried to unresolve reference for declaration that doesn't exist in the graph")
                }
            }
    ...
    pub fn record_resolved_reference(&mut self, reference_id: ConstantReferenceId, declaration_id: DeclarationId) {
        match self.declarations.get_mut(&declaration_id) {
            Some(Declaration::Namespace(it)) => {
                it.add_reference(reference_id);
            }
            Some(Declaration::Constant(it)) => {
                it.add_reference(reference_id);
            }
            ...
            Some(
                ...
                unreachable!("Cannot record constant reference for something that isn't a constant")
            }
            None => {
                panic!("Tried to record a constant reference for a declaration that doesn't exist")
            }
        }
    }
    ...
    fn remove_document_data(&mut self, document: &Document) {
        ...
        for ref_id in document.constant_references() {
            if let Some(constant_ref) = self.constant_references.remove(ref_id) {
                ...
                {
                    match declaration {
                        Declaration::Namespace(it) => {
                            it.remove_reference(ref_id);
                        }
                        Declaration::Constant(it) => {
                            it.remove_reference(ref_id);
                        }
                        Declaration::ConstantAlias(it) => {
                            it.remove_reference(ref_id);
                        }
                        Declaration::Method(_)
                        | Declaration::GlobalVariable(_)
                        | Declaration::InstanceVariable(_)
                        | Declaration::ClassVariable(_) => {
                            unreachable!("Cannot remove constant reference ID from non constant declaration")
                        }
                    }
                }
    ...
    fn names_for_declaration(&self, decl_id: DeclarationId) -> IdentityHashSet<NameId> {
        ...
        let empty = IdentityHashSet::default();
        let references = match decl {
            Declaration::Namespace(it) => it.references(),
            Declaration::Constant(it) => it.references(),
            Declaration::ConstantAlias(it) => it.references(),
            Declaration::Method(_)
            | Declaration::GlobalVariable(_)
            | Declaration::InstanceVariable(_)
            | Declaration::ClassVariable(_) => &empty,
        };
