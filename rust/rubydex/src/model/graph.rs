...
mod incremental_resolution_tests {
    ...
    fn assert_no_stale_descendants(context: &GraphTest, declaration_name: &str) {
        let declaration = context
            .graph()
            .declarations()
            .get(&crate::model::ids::DeclarationId::from(declaration_name))
        ...
        for id in namespace.descendants() {
            assert!(
                context.graph().declarations().contains_key(id),
                "{declaration_name} has stale descendant id {id:?} with no backing declaration"
            );
        }
    }
    ...
    #[test]
    fn deleting_top_level_declaration_removes_stale_object_descendant() {
        let mut context = GraphTest::new();
        ...
        assert_no_stale_descendants(&context, "Kernel");
    }

    #[test]
    fn empty_singleton_class_chain_removed_after_def_self_becomes_instance_method() {
        let mut context = GraphTest::new();
    ...
    #[test]
    fn singleton_class_kept_when_definition_removed_but_reference_remains() {
        let mut context = GraphTest::new();
        context.index_uri(
            "file:///foo.rb",
            "
