...
use rubydex::model::declaration::{Ancestor, Declaration};
use rubydex::model::ids::DeclarationId;
use rubydex::resolution::ancestors_of;
...
pub unsafe extern "C" fn rdx_declaration_ancestors_iter_new(
    ...
    with_graph(pointer, |graph| {
        let decl_id = DeclarationId::new(decl_id);
        // Use ancestors_of to compute ancestors on-demand if not already cached
        let ancestors_result = ancestors_of(graph, decl_id);
        let ancestors: Vec<i64> = ancestors_result
            .iter()
            .filter_map(|ancestor| {
