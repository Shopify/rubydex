  ...
  module Server
    ...
    class Core
      ...
      def run
        ...
        # Record the pid before the slow index/resolve so the parent can tell a boot crash apart
        # from a slow start and fail fast instead of waiting out the boot timeout.
        @cache.write_pid!
        ...
        # Surface boot/serve failures to the log; without this a detached crash is swallowed and
        # looks like a slow start to the client.
      ...
      def workspace_manifest
        ...
        collect_files(@cache.workspace_path, manifest, patterns)
        manifest
      end

      #: (String dir, Hash[String, Float] manifest, Array[String] patterns) -> void
      def collect_files(dir, manifest, patterns)
        Dir.each_child(dir) do |entry|
          full = File.join(dir, entry)
          next if excluded_by_patterns?(full, patterns)

          if File.directory?(full)
            collect_files(full, manifest, patterns)
      ...
      def redirect_output
        ...
        target = override && !override.empty? ? override : @cache.log_path
