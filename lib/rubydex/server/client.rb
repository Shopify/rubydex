      ...
      class << self
        ...
        def wait_until_ready(cache)
          ...
          until File.socket?(cache.socket_path) && cache.version_compatible?
            # The daemon records its pid before the slow boot work. Once we've seen it, a pid that
            # dies (or a pid file cleaned up on crash) means the server failed to boot — fail fast
            # instead of waiting out the full timeout.
            pid = cache.server_pid
            seen_pid ||= !pid.nil?
            if seen_pid && (pid.nil? || !process_alive?(pid))
              raise Error, "the rdx server exited during startup; see #{server_log_hint(cache)} for details"
            end

            raise Error, "timed out waiting for the rdx server to start (see #{server_log_hint(cache)})" if monotonic > deadline

            sleep(0.05)
          end
        end

        # Where the detached server writes its output, for pointing users at boot failures.
        #: (Cache cache) -> String
        def server_log_hint(cache)
          override = ENV["RDX_SERVER_LOG"]
          override && !override.empty? ? override : cache.log_path
        end
