...
static const rb_data_type_t index_type = {"Index",
                                          {
                                              0,
                                              index_free,
                                              0,
...
static VALUE rb_index_new(VALUE klass) {
  ...
  // TODO: Need to determine a potential way to allow interaction with the index
...
static VALUE rb_repository_get_entry(VALUE self, VALUE name) {
  ...
  rb_ivar_set(ruby_entry, rb_intern("@name"),
              entry_name ? rb_str_new_cstr(entry_name) : Qnil);
  rb_ivar_set(ruby_entry, rb_intern("@value"),
              entry_value ? rb_str_new_cstr(entry_value) : Qnil);
