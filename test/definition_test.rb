...
class DefinitionTest < Minitest::Test
  ...
  def test_definition_source_name_can_build_constant_resolution_nesting
    with_context do |context|
      context.write!("file1.rb", <<~RUBY)
        class Bar
          class Baz; end
      ...
      bar_def = definitions.find do |definition|
        definition.is_a?(Rubydex::ClassDefinition) && definition.source_name == "::Bar"
      end
      nesting = bar_def.lexical_nesting.reverse.map(&:source_name) << bar_def.source_name
