...
class DefinitionTest < Minitest::Test
  ...
  def test_method_definition_signatures_with_various_parameter_kinds
    with_context do |context|
      ...
      assert_equal("#{path}:1:9-1:10", params[0][2].to_display.to_s) # a

      assert_equal([:opt, :b], params[1][0..1])
      assert_equal("#{path}:1:12-1:13", params[1][2].to_display.to_s) # b

      assert_equal([:rest, :c], params[2][0..1])
      assert_equal("#{path}:1:20-1:21", params[2][2].to_display.to_s) # c

      assert_equal([:req, :d], params[3][0..1])
      assert_equal("#{path}:1:23-1:24", params[3][2].to_display.to_s) # d

      assert_equal([:keyreq, :e], params[4][0..1])
      assert_equal("#{path}:1:26-1:28", params[4][2].to_display.to_s) # e:

      assert_equal([:key, :f], params[5][0..1])
      assert_equal("#{path}:1:30-1:32", params[5][2].to_display.to_s) # f:

      assert_equal([:keyrest, :g], params[6][0..1])
      assert_equal("#{path}:1:38-1:39", params[6][2].to_display.to_s) # g

      assert_equal([:block, :h], params[7][0..1])
      assert_equal("#{path}:1:42-1:43", params[7][2].to_display.to_s) # h
    end
  end
  ...
  def test_method_definition_signatures_forward
    with_context do |context|
      ...
      assert_equal(:forward, params[0][0])
      assert_equal(:"...", params[0][1])
      assert_equal("#{path}:1:9-1:12", params[0][2].to_display.to_s) # ...
    end
  end
