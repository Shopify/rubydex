  ...
  module Server
    ...
    class IntegrationTest < Minitest::Test
      ...
      def test_boots_and_indexes_files_in_subdirectories
        with_context do |context|
          track(context)
          # Nested directories exercise the workspace manifest's recursive walk — the path that
          # previously crashed the daemon at boot on a real (non-flat) codebase.
          ...
          assert_match(/Dog/, output)
          # The server booted (past the manifest walk) and stayed resident.
          assert(server_alive?(context))
        end
      end
